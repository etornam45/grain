...
func diffEnums(old, new Snapshot, prompt PromptFunc) ([]Change, error) {
	...
	// Resolve removed/renamed enums first so a rename target isn't also
...
func diffTables(old, new Snapshot, prompt PromptFunc) ([]Change, error) {
	...
	for _, name := range sortedKeys(newTables) {
		...
		idxChanges, err := diffIndexes(name, oldT, newT)
		if err != nil {
			return nil, err
		}
		uniqueChanges, err := diffUniques(name, oldT, newT)
		if err != nil {
			return nil, err
		}
		constraintChanges, err := diffConstraints(name, oldT, newT)
		if err != nil {
			return nil, err
		}
	...
	for _, oldName := range unmatchedOld {
		...
		case "rename":
			...
			changes = append(changes, idxChanges...)
			uniqueChanges, err := diffUniques(res.Target, oldTables[oldName], newTables[res.Target])
			if err != nil {
				return nil, err
			}
			changes = append(changes, uniqueChanges...)
			constraintChanges, err := diffConstraints(res.Target, oldTables[oldName], newTables[res.Target])
			if err != nil {
				return nil, err
			}
			changes = append(changes, constraintChanges...)
