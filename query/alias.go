...
// isSchemaColumn reports whether a colRef is a plain schema column (renders as
// "table.col") rather than a raw expression. Raw/aggregate columns go through
// ColRef and must be emitted verbatim.
func isSchemaColumn(c colRef) bool {
	_, ok := c.(*schema.ColumnDef)
	return ok
}